...
public class FlyBehaviour : GenericBehaviour
{
	...
	public bool IsFlying { get; private set; }                     // Boolean to determine whether or not the player activated fly mode.
	...
	void Update()
	{
			...
			&& !behaviourManager.GetTempLockStatus(behaviourManager.GetDefaultBehaviour))
		{
			...
			behaviourManager.GetRigidBody.useGravity = !IsFlying;
		...
		IsFlying = IsFlying && behaviourManager.IsCurrentBehaviour(this.behaviourCode);

		// Set fly related variables on the Animator Controller.
		behaviourManager.GetAnim.SetBool(flyBool, IsFlying);
	}
